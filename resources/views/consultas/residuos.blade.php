...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <form method="POST" action="{{ route('consultas.consulta.postResiduo') }}" accept-charset="UTF-8" id="create_condicione_form" name="create_condicione_form" class="">
            {{ csrf_field() }}
        ...
        <div class="form-group col-md-6 {{ $errors->has('residuo_f') ? 'has-error' : '' }}">
            <label for="residuo_f" class="control-label">Residuo de:</label>
            <!--<div class="col-md-10">-->
                <select class="form-control chosen" id="residuo_f" name="residuo_f" required="true">
                            <option value="" style="display: none;"  disabled selected>Seleccionar</option>
                                @foreach ($residuos_ls as $key => $cia)
                                    <option value="{{ $key }}">
                                        {{ $cia }}
                                    </option>
                                @endforeach
                ...
                {!! $errors->first('residuo_id', '<p class="help-block">:message</p>') !!}
            <!--</div>-->
        </div>    
        
        <div class="form-group col-md-6 {{ $errors->has('residuo_t') ? 'has-error' : '' }}">
            <label for="residuo_t" class="control-label">Residuo a:</label>
            <!--<div class="col-md-10">-->
                <select class="form-control chosen" id="residuo_t" name="residuo_t" required="true">
                            <option value="" style="display: none;"  disabled selected>Seleccionar</option>
                                @foreach ($residuos_ls as $key => $cia)
                                    <option value="{{ $key }}">
                                        {{ $cia }}
