...
<div class="form-group col-md-4 {{ $errors->has('anio') ? 'has-error' : '' }}">
    ...
    <!--<div class="col-md-10">-->
        <input class="form-control input-sm " name="anio" type="number" id="anio" value="{{ old('anio', optional($revDocumental)->anio) }}" min="-2147483648" max="2147483647" required="true" placeholder="{{ trans('rev_documentals.anio__placeholder') }}">
...
<div class="form-group col-md-4 {{ $errors->has('mes_id') ? 'has-error' : '' }}">
    ...
    <!--<div class="col-md-10">-->
        <select class="form-control chosen" id="mes_id" name="mes_id" required="true">
        	    <option value="" style="display: none;" {{ old('mes_id', optional($revDocumental)->mes_id ?: '') == '' ? 'selected' : '' }} disabled selected>{{ trans('rev_documentals.mes_id__placeholder') }}</option>
        	@foreach ($mese as $key => $mese)
			    <option value="{{ $key }}" {{ old('mes_id', optional($revDocumental)->mes_id) == $key ? 'selected' : '' }}>
			    	{{ $mese }}
			    </option>
			@endforeach
        </select>
